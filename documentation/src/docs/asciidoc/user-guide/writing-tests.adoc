...
Unquoted leading and trailing whitespaces are trimmed by default, but this behavior can be changed by setting the `ignoreTrailingAndLeadingWhitespace` attribute to `true`.
...
| `@CsvSource(value = { " apple , banana" }, ignoreTrailingAndLeadingWhitespace = false)` | `" apple "`, `" banana"`
|===
